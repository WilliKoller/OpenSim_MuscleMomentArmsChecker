...
coordIndMod = zeros(1, numel(coordinateNames));
...
for i = 1 : numel(coordinateNames)
    ...
    coordIndMod(i) = modelModified.getCoordinateSet().getIndex(coordinateNames{i});
    coordinateHandlesModified{i} = modelModified.updCoordinateSet().get(coordIndMod(i));
end
...
for i = 0 : numMuscles - 1
    ...
    if contains(tmp_muscleName, muscleFilter)
        ...
        if strcmp(tmp_muscleName, tmp_muscleNameMod)
            ...
            disp(['Muscle IDs of ' tmp_muscleName ' are not the same, probably the models are modified extensively.']);
            return;
        end
    end
end
...
for u = 1 : numel(motionFileNames)
    ...
    numberOfMuscles = size(momentArmsCurrMotionModified, 2);
